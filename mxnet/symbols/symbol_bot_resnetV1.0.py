...
def MyResidualFactory(data, num_filter, dim_match):
...
def ResidualNet(data, nBlocks):
    ...
    for i in range(nBlocks[0]):
        data = MyResidualFactory(data=data, num_filter=64, dim_match=True)
    
    #second blocks
    for i in range(nBlocks[1]):
        if i==0:
            data = MyResidualFactory(data=data, num_filter=128, dim_match=False)
        else:
            data = MyResidualFactory(data=data, num_filter=128, dim_match=True)

    #third blocks
    for i in range(nBlocks[2]):
        if i==0:
            data = MyResidualFactory(data=data, num_filter=192, dim_match=False)
        else:
            data = MyResidualFactory(data=data, num_filter=192, dim_match=True)

    #fourth blocks
    for i in range(nBlocks[3]):
        if i==0:
            data = MyResidualFactory(data=data, num_filter=256, dim_match=False)
        else:
            data = MyResidualFactory(data=data, num_filter=256, dim_match=True)
