...
def pad_data_to_max_shape(data, max_shape):
    padded_data = np.zeros(max_shape, np.float)
    height, width = data.shape[2],data.shape[3]
    top, left = 0, 0
    padded_data[..., top:top + height, left:left + width] = data
    return padded_data
...
def get_image_array(roidb, scales, scale_indexes):
    ...
    for i in range(num_images):
        ...
        im_tensor = pad_data_to_max_shape(im_tensor,config.MAX_SHAPE)
