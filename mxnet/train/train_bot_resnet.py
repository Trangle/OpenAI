...
With #2366 and a batch size of 64, I got an accuracy of 0.939704 after 200 epochs on 2 GPUs, at epoch 483 get accuracy 0.941807.
...
parser = argparse.ArgumentParser(description='train an image classifer on kesci_bot')
...
parser.add_argument('--model-prefix', type=str,default= "./models/resnet-52",
...
parser.add_argument('--log-file', type=str, default='log-resnet', help='the name of log file')
parser.add_argument('--log-dir', type=str, default='Logs', help='directory of the log file')
args = parser.parse_args()

# network
import importlib
# net = importlib.import_module('symbol_' + args.network).get_symbol(num_class=args.num_classes,num_level=4,num_block=20)
net = importlib.import_module('symbol_' + args.network).get_symbol(num_class=args.num_classes)
...
def fit(args, network, data_loader, batch_end_callback=None):
    ...
    model = mx.model.FeedForward(
        ...
        , lr_scheduler=Scheduler(epoch_step=[80, 160], factor=args.lr_factor, epoch_size=epoch_size)
        , **model_args)

    eval_metrics = []
    # eval_metrics.append(mx.metric.np(LogLoss))
    ## TopKAccuracy only allows top_k > 1
    # for top_k in [2, 3, 5]:
    #     eval_metrics.append(mx.metric.create('top_k_accuracy', top_k=top_k))
    # eval_metrics.append('ce')
    ...
    if batch_end_callback is not None:
        ...
        batch_end_callback = []
    batch_end_callback.append(mx.callback.Speedometer(args.batch_size, 20))
