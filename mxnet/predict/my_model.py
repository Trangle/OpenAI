...
class FeedForward(BASE_ESTIMATOR):
    ...
    def predict(self, X, multi_size=None, num_batch=None, return_data=False, reset=True):
        ...
        for batch in X:
            ...
            j = 0
            for o_nd in self._pred_exec.outputs:
                ret =(o_nd[0:real_size].asnumpy(), _, _)
                if return_data:
                    ret = (o_nd[0:real_size].asnumpy(), batch.data[j][0:real_size].asnumpy(), batch.label[j][0:real_size].asnumpy())
                j += 1
                yield ret
