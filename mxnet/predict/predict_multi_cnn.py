...
model_prefixes = ['D:/Data/Image/bot_animal/models/net_incep-res-3', 'D:/Data/Image/bot_animal/models/net_incep-res-2']
...
def read_files(file_pth, header):
    ...
    ret = img_lst.values.tolist()
    # map(lambda x: ret.setdefault(x[0], x[1]), img_lst)
    # map(lambda x: ret.append(x[:2]), img_lst)
    return ret
...
def parse_args():
    ...
    parser.add_argument('--data-dir', type=str, default="D:/Data/Image/bot_animal/data/mean/",
                        help='the input data directory')
    parser.add_argument('--test-dir', type=str, default="D:/Data/Image/bot_animal/data/vali/",
                        help='the input data directory')
    parser.add_argument('--dest-dir', type=str, default="D:/Data/Image/bot_animal/predict_data/",
                        help='the output data directory')
    parser.add_argument('--gpus', type=str, default='0',
                        help='the gpus will be used, e.g "0,1,2,3"')
    parser.add_argument('--model-prefix', type=str, default="D:/Data/Image/bot_animal/models/net_incep-res-3",
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i, x in enumerate(ret):
            prob = prediction[i]
            tmp_key = str(x[2]) + ' ' + str(x[1])
            if pre.has_key(tmp_key):
                pre[tmp_key] += prob
            else:
                pre[tmp_key] = prob
        csvfile = open('submission.txt', 'wb')
        ...
        for d, x in pre.items():
            ...
            count += 1
            tmp_pre = topk(pred, 2)
            y = ['%.6f' % v for v in tmp_pre]
            index = [pred.index(i) for i in tmp_pre]
            parts = d.split(' ')
            curr_label = int(parts[1])
            if index[0] == curr_label or index[1] == curr_label:
                correct[1] += 1
                if index[0] == curr_label:
                    correct[0] += 1
            root, file = os.path.split(parts[0])
            file_name, ext = os.path.splitext(file)
            tmp_row = list(chain(*zip(index, y)))
            tmp_row.insert(0, file_name)
